...
async def shutdown_event():
    """Disconnect from database on shoutdown"""
...
async def extract_aircraft_data(file:UploadFile=File(...,description="PDF file containing aircraft utilization report")):
     """
     ...
     try:
         ...
         if not is_valid:
             logger.warning(f"⚠️ Validation warning :{len(warnings)}")
        # storing data in database 
         logger.info("📤Storing extract_data in Database...")
...
async def get_aircraft_by_id(record_id:str):
    ...
    try:
        ...
        if not aircraft_data:
            raise HTTPException(
                status_code=404,
                detail=f"Aircraft record  not found {record_id}"
            )
        return {
            "success":True,
            "data":aircraft_data
        ...
        logger.error(f"❌ Error retriving aircraft data :{str(e)}")
        raise HTTPException(
            status_code=500,
